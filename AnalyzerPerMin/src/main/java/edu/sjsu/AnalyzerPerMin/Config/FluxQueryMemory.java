...
public class FluxQueryMemory extends DbConnections {
    ...
    public static void setRange() {
     	log.debug("Inside setRange");
   	 	...
   	 	String query1 = "from (bucket:\"KPIDB-LOW/autogen\") |> range(start: -100d) |> filter(fn: (r) => r._measurement == \"MemoryPerMinTopicLowLoad\" and r._field == \"UTC\") |>  last()";
   	 	...
   	 	if (tables1.isEmpty()==true){
   	 		...
   	 		//List<FluxRecord> records2 = tables2.get(0).getRecords();
   	 		...
   	 		//List<FluxRecord> records1 = tables1.get(0).getRecords();
	    ...
	    public static void performQuery() throws NotEnoughData, InterruptedException {
	    	 ...
	    	 String query = "from (bucket:\"KPIDB-LOW/autogen\") |> range(start: "+ start.toString()+ " , stop: " + stop.toString()+ ") |> filter(fn: (r) => r._measurement == \"Stats\" and r._field == \"Mem_Percentage\")|> group(columns: [\"FQDN\"], mode:\"by\") |> window(every: 1m) |> mean()|> duplicate(column: \"_stop\", as: \"_time\") |> yield()";
...
//	            	 System.out.println(fluxRecord.getTime() + ": " + fluxRecord.getValueByKey("_value"));
	            	 Point p = Point.measurement("MemoryPerMinTopicLowLoad")
